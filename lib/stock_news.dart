...
class _StockNewsPageState extends State<StockNewsPage>
  ...
  Future<void> _initializePurchasedStocks() async {
    try {
      ...
      if (user != null) {
        final DocumentSnapshot userInvestmentsDoc = await FirebaseFirestore.instance
            .collection('user_investments')
            .doc(user.uid)
            .get();

        if (userInvestmentsDoc.exists) {
          setState(() {
            purchasedStocks = (userInvestmentsDoc.data() as Map<String, dynamic>).keys.toList();
          });
        }
      }
    } catch (e) {
